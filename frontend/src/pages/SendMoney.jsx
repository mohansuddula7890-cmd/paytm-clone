...
export const SendMoney=()=>{
    ...
    const handleTransfer=async ()=>{
        if(!isAmountValid()){
            setError("Please enter valid Whole number amount greater than 0");
            return;
        }
        setLoading(true)
        setError("");
        setSuccess("");
        // process.env.VITE_API_URL is for node.js
        try{
            const response=await axios.post(`${import.meta.env.VITE_API_URL}/api/v1/account/transfer`,
                {
                    userId:id,
